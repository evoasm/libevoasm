...
typedef enum {
  EVOASM_KERNEL_IO_VAL_TYPE_I64,
  EVOASM_KERNEL_IO_VAL_TYPE_U64,
  EVOASM_KERNEL_IO_VAL_TYPE_F64,
...
typedef struct {
  uint8_t arity;
  uint16_t len;
  evoasm_kernel_io_val_type_t types[EVOASM_PROGRAM_IO_MAX_ARITY];
  evoasm_kernel_io_val_t vals[1];
} evoasm_kernel_io_t;
...
evoasm_kernel_io_get_n_tuples(evoasm_kernel_io_t *program_io) {
