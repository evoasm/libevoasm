...
#define EVOASM_DEME_TOPOLOGIES_ROW(deme) ((deme)->params->n_kernels)

#define EVOASM_DEME_N_TOPOLOGY_EDGES_PER_PROGAM_(n_kernels) (4u * n_kernels)
#define EVOASM_DEME_N_TOPOLOGY_EDGES_PER_PROGAM(deme) EVOASM_DEME_N_TOPOLOGY_EDGES_PER_PROGAM_(deme->params->n_kernels)

#define EVOASM_DEME_TOPOLOGY_EDGE_OFF_(n_kernels, topology_idx, edge_idx) \
  (3u * (((topology_idx) * EVOASM_DEME_N_TOPOLOGY_EDGES_PER_PROGAM_(n_kernels)) + (edge_idx)))

#define EVOASM_DEME_TOPOLOGY_EDGE_OFF(deme, topology_idx, edge_idx) \
  EVOASM_DEME_TOPOLOGY_EDGE_OFF_((deme)->params->n_kernels, topology_idx, edge_idx)
...
static evoasm_success_t
evoasm_pop_topology_data_init(evoasm_pop_topology_data_t *topology_data, size_t n_topologies, size_t n_kernels) {
  ...
  EVOASM_TRY_ALLOC(error, aligned_calloc, topology_data->edges, EVOASM_CACHE_LINE_SIZE,
                   n_topologies * EVOASM_DEME_N_TOPOLOGY_EDGES_PER_PROGAM_(n_kernels) * 3,
                   sizeof(uint8_t));
  return true;
...
evoasm_pop_params_get_deme_height_(evoasm_pop_params_t *params) {
  size_t height = params->n_kernels;
  if(params->n_kernels > 1) return height + 1;
  return height;
}
...
evoasm_deme_init(evoasm_deme_t *deme,
  ...
  *deme = zero_deme;
  ...
  if(params->n_kernels > 1) {
    ...
    EVOASM_TRY(error, evoasm_pop_topology_data_init, &deme->best_topology_data, 1u, params->n_kernels);
  }
  ...
  EVOASM_TRY(error, evoasm_pop_kernel_data_init, &deme->kernel_data, arch_id,
             (size_t) params->n_kernels * params->deme_size, params->kernel_size);
  ...
  EVOASM_TRY(error, evoasm_pop_kernel_data_init, &deme->best_kernel_data, arch_id,
             params->n_kernels, params->kernel_size);
...
evoasm_deme_mutate_topology_edge(evoasm_deme_t *deme,
  ...
  size_t n_kernels = deme->params->n_kernels;
  evoasm_prng_t *prng = &deme->prng;
  ...
  uint8_t kernel_idx = (uint8_t) evoasm_prng_rand_between_(prng, 0, (int64_t) n_kernels);
  uint8_t succ_kernel_idx = (uint8_t) evoasm_prng_rand_between_(prng, 0, (int64_t) n_kernels);
...
evoasm_deme_seed_default_topology_edges(evoasm_deme_t *deme,
  ...
  for(size_t i = backbone_len - 1; i < n_kernels - 1; i++) {
    size_t edge_off = EVOASM_DEME_TOPOLOGY_EDGE_OFF(deme, topology_idx, i);
...
evoasm_deme_seed_topology(evoasm_deme_t *deme,
  ...
  for(size_t i = n_kernels - 1; i < EVOASM_DEME_N_TOPOLOGY_EDGES_PER_PROGAM_(n_kernels); i++) {
    uint8_t kernel_idx = (uint8_t) evoasm_prng_rand_between_(prng, 0, (int64_t) n_kernels);
    uint8_t succ_kernel_idx = (uint8_t) evoasm_prng_rand_between_(prng, 0, (int64_t) n_kernels);
    uint8_t cond = (uint8_t) evoasm_prng_rand_between_(prng, 0, UINT8_MAX);
...
//  for(size_t i = 0; i < deme->params->n_kernels; i++) {
...
evoasm_deme_update_best(evoasm_deme_t *deme, evoasm_loss_t loss, size_t topology_idx, size_t *kernel_idxs) {
  ...
  if(loss < deme->best_loss) {
    ...
    size_t dst_edge0_off = EVOASM_DEME_TOPOLOGY_EDGE_OFF_(deme->params->n_kernels, 0, 0);
...
evoasm_deme_get_trivial_mutate_crossover_gene_range(evoasm_deme_t *deme, size_t *start, size_t *len,
  ...
  if(topologies) {
    size_t n_kernels = deme->params->n_kernels;
    *start = n_kernels;
    *len = EVOASM_DEME_N_TOPOLOGY_EDGES_PER_PROGAM_(n_kernels);
  } else {
    *start = 0;
...
evoasm_deme_inject_best(evoasm_deme_t *deme, evoasm_deme_t *src_deme, size_t row, bool topology) {
  ...
  if(topology) {
    size_t src_edge0_off = EVOASM_DEME_TOPOLOGY_EDGE_OFF_(deme->params->n_kernels, 0, 0);
    ...
    evoasm_pop_topology_data_copy_topology_edges(&src_deme->best_topology_data, src_edge0_off, &deme->topology_data,
                                                 dst_edge0_off, deme->params->n_kernels);
  } else {
    ...
    evoasm_pop_kernel_data_copy(&src_deme->best_kernel_data, deme->arch_id, src_inst0_off, &deme->kernel_data,
                                dst_inst0_off, deme->params->n_kernels);
  }
